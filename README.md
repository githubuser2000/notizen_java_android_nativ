...
- Neuer Button `Stift` in der Textleiste: öffnet ein weißes Schreibfeld für Samsung-Stift oder Finger und fügt das Ergebnis als normales eingebettetes RTF-Bild ein.
- Stiftbilder laufen durch dieselbe speichersichere Bildkompression wie normale eingefügte Bilder.
- RTF-Kontextmenü ergänzt um `Stiftbild einfügen…`.
- Android-Haftnotiz-Widgets zeigen jetzt eine Bildvorschau aus RTF-Bildern des Knotens.
- Mehrere RTF-Bilder werden als kleine Collage in einem Widget-Bildfeld zusammengefasst.
...
- Markdown-Vorschau auf HTML/WebView im RTF-Bereich umgestellt, damit Tabellen und erweiterte Markdown-Blöcke sichtbar gerendert werden können.
